// file: WhisperLib.c
// ============================================================
// ✅ whisper.cpp JNI Bridge — Safe Loader + Transcriber
// ------------------------------------------------------------
 ...
 * - attached_by_us: set if current thread attached by native side (during read)
 */
...
static size_t is_read(void *ctx, void *output, size_t read_size) {
    ...
    const jint chunk = (jint)((read_size > (size_t)is->buf_len) ? is->buf_len : (jint)read_size);

    const jint n = (*env)->CallIntMethod(
            env,
            is->input_stream,
            is->mid_read,
            (jbyteArray)is->buffer_gl,
            0,
            chunk
    );
...
static void is_close(void *ctx) {
    ...
    // Detach if we attached in is_read() OR attached in this close() function.
    if (is->jvm && (is->attached_by_us || attached_here)) {
...
Java_com_whispercpp_whisper_WhisperLib_initContextFromInputStream(
    ...
    if (!input_stream) {
        LOGW("initContextFromInputStream: InputStream NULL");
        return 0;
    }
    ...
    if (!ctx) {
        ...
        // IMPORTANT:
        // whisper.cpp is expected to call loader.close(loader.context) on failure as well.
        // Calling is_close(inp) here risks double-free.
        return 0;
    }
...
static struct whisper_context* whisper_init_from_asset(
    ...
    if (!mgrObj || !asset_path) {
        LOGW("Invalid asset arguments");
        return NULL;
    }
    ...
    struct whisper_context *ctx = whisper_init_with_params(&loader, cparams);
    if (!ctx) {
        LOGE("whisper_init_with_params() failed (Asset)");
    }
    return ctx;
}
...
Java_com_whispercpp_whisper_WhisperLib_initContext(
    ...
    if (!ctx) {
        ...
        // IMPORTANT: log before ReleaseStringUTFChars() to avoid use-after-free.
...
Java_com_whispercpp_whisper_WhisperLib_fullTranscribe(
    ...
    if (!ctx || !audio) {
        LOGW("fullTranscribe: context or audio NULL");
        return;
    }

    jfloat *pcm = (*env)->GetFloatArrayElements(env, audio, NULL);
    if (!pcm) {
        LOGE("GetFloatArrayElements() failed");
        return;
    }
    ...
    if (langStr) {
        lang = (*env)->GetStringUTFChars(env, langStr, NULL);
    }
    ...
    LOGI("Starting whisper_full(): samples=%d threads=%d translate=%d",
         (int)n, p.n_threads, p.translate);
    ...
    if (langStr && lang) {
        (*env)->ReleaseStringUTFChars(env, langStr, lang);
    }
